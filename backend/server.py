...
# # Days after a wedding is marked "completed" before its media is auto-purged.
# # None means never auto-delete (Premium keeps media forever).
# RETENTION_DAYS = {"basic": 7, "pro": 60, "premium": 90}
...
RETENTION_DAYS = {"basic": 7, "pro": 90, "premium": None}
...
def set_auth_cookies(response: Response, access: str, refresh: str):
    ...
    same_site = "none" if secure_cookies else "lax"
    response.set_cookie(
        "access_token", access, httponly=True, secure=secure_cookies,
        samesite=same_site, max_age=43200, path="/",
    )
    response.set_cookie(
        "refresh_token", refresh, httponly=True, secure=secure_cookies,
...
async def maybe_send_storage_warning(current_bytes: int):
    ...
    try:
        await send_email(
            ...
            f"<p>Total gallery storage has crossed {STORAGE_WARN_THRESHOLD_BYTES / (1024**3):.0f}GB "
            f"(currently {current_bytes / (1024**3):.2f}GB). Uploads will be blocked at "
            f"{STORAGE_HARD_LIMIT_BYTES / (1024**3):.0f}GB.</p>"
        )
    except Exception as e:
        logger.error(f"storage warning email failed: {e}")
    await db.system_flags.update_one(
...
# ---------------------------------------------------------------- media retention (auto-purge)
async def purge_expired_weddings():
    now = datetime.now(timezone.utc)
    completed = await db.weddings.find({"status": "completed", "media_purged": {"$ne": True}}).to_list(10000)
    for w in completed:
        tier = w.get("upload_tier", "basic")
        ...
        try:
            ...
            continue
        if now - completed_dt < timedelta(days=days):
            continue

        slug = w["slug"]
        uploads = await db.uploads.find({"wedding_slug": slug, "is_deleted": False}).to_list(10000)
        for u in uploads:
            ...
            if path:
                try:
                    ...
                    logger.warning(f"purge: could not delete object for wedding={slug}, path={path}: {e}")
        await db.uploads.update_many({"wedding_slug": slug}, {"$set": {"is_deleted": True}})
        await db.weddings.update_one(
            {"_id": w["_id"]},
            {"$set": {"media_purged": True, "media_purged_at": now_iso(), "upload_count": 0}},
        )
        logger.info(f"Purged media for completed wedding {slug} (tier={tier}, retention={days}d)")
...
def wedding_public(w: dict) -> dict:
    return {
        ...
        "upload_count": w.get("upload_count", 0),
        "upload_tier": w.get("upload_tier", "basic"),
        "upload_limit": w.get("upload_limit", WEDDING_UPLOAD_TIERS["basic"]["limit"]),
        "created_at": w.get("created_at"),
    }
...
async def update_wedding(slug: str, data: UpdateWeddingInput, user: dict = Depends(require_role("restaurant", "admin"))):
    ...
    if data.upload_tier is not None:
        if data.upload_tier not in WEDDING_UPLOAD_TIERS:
            raise HTTPException(status_code=400, detail="Invalid upload tier")
        updates["upload_tier"] = data.upload_tier
        updates["upload_limit"] = WEDDING_UPLOAD_TIERS[data.upload_tier]["limit"]
...
async def guest_upload(slug: str, request: Request, file: UploadFile = File(...), guest_name: str = Form("")):
    ...
    wedding_limit = w.get("upload_limit", WEDDING_UPLOAD_TIERS["basic"]["limit"])
    if w.get("upload_count", 0) >= wedding_limit:
        raise HTTPException(
            status_code=403,
            detail=f"This gallery has reached its {wedding_limit}-photo limit. Please contact the venue.",
...
async def startup():
    ...
    scheduler.start()
    logger.info("Media retention scheduler started")
